...
export function CartScreen() {
  ...
  const { cart, updateQty, removeFromCart, cartTotal, coupon, discount, applyCoupon } = useApp();
            ...
            <div className="flex flex-col gap-3 mb-4">
              {cart.map(item => (
                <div key={item.product.id} className="bg-white rounded-2xl p-3 flex items-center gap-3 shadow-sm">
                  <img src={item.product.image} alt={item.product.name} className="rounded-xl object-cover flex-shrink-0"
