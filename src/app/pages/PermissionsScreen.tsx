...
export function PermissionsScreen() {
  ...
  useEffect(() => {
    ...
    const loadLocationPermission = async () => {
      ...
      try {
        browserPermission = await navigator.permissions.query({
          name: "geolocation",
        });
        if (!isActive) return;
  ...
  const handleEnablePush = async () => {
    if (!isLoggedIn) {
      navigate(tenantPath("login"), {
        state: { redirectTo: tenantPath("privacy/permissions") },
      });
      return;
    }
    ...
    try {
      ...
      setPushPermission(getPushPermission());
      showSystemNotice(
        error?.message || "Não foi possível ativar notificações.",
      );
    } finally {
  ...
  const handleDisablePush = async () => {
    ...
    try {
      ...
      showSystemNotice(
        error?.message || "Não foi possível desativar notificações.",
      );
    } finally {
      setPushRegistered(hasCustomerPushRegistration());
      setChangingPush(false);
  ...
  const handleTogglePreference = async (
    field: "orders_enabled" | "campaigns_enabled",
  ) => {
    if (!pushRegistered || !preferences) {
      showSystemNotice(
        "Ative as notificações push para escolher as categorias.",
      );
      return;
    }
    ...
    try {
      const updated = await updateCustomerNotificationPreferences({
      ...
      showSystemNotice(
        error?.message || "Não foi possível atualizar suas preferências.",
      );
    } finally {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        if (error.code === error.PERMISSION_DENIED) {
          ...
          showSystemNotice(
            "A localização foi bloqueada. Libere o acesso nas configurações do navegador ou do app instalado.",
          );
        } else {
          showSystemNotice(
            "Não foi possível confirmar sua localização agora. Tente novamente em alguns instantes.",
          );
        }
        setCheckingLocation(false);
      },
  ...
  const canEditPreferences = isLoggedIn && pushRegistered && Boolean(preferences);
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p
                className="mt-1"
                style={{ fontSize: 12, color: "#64748b", lineHeight: 1.45 }}
              >
              ...
              {pushPermission === "granted" && !pushRegistered && (
                <p
                  className="mt-2"
                  style={{ fontSize: 12, color: "#a16207", fontWeight: 700 }}
                >
                  Permitida no navegador, mas ainda não ativada neste
                  dispositivo.
                </p>
              )}
              {pushRegistered && (
                ...
                >
                  <CheckCircle2 size={14} /> Dispositivo cadastrado para receber
                  push.
                </p>
              )}
            </div>
          ...
          {pushRegistered ? (
            ...
            >
              <BellOff size={17} />{" "}
              {changingPush ? "Desativando..." : "Desativar neste dispositivo"}
            </button>
          ) : (
            <button
          ...
          >
            ...
            Desativar aqui remove este dispositivo do envio de push, mas não
            revoga a autorização salva pelo navegador.
          </p>
        </section>
        ...
        >
          ...
          <p
            ...
            style={{ fontSize: 12, color: "#64748b", lineHeight: 1.45 }}
          >
            O histórico dentro do app continua disponível mesmo quando o push
            estiver desligado.
          </p>

          {[
            {
              field: "orders_enabled" as const,
              title: "Atualizações de pedidos",
              description:
                "Confirmação, separação, entrega ou retirada e demais mudanças do pedido.",
            },
            {
              field: "campaigns_enabled" as const,
              title: "Campanhas e promoções",
              description:
                "Ofertas e campanhas enviadas pela loja para o seu perfil.",
            },
          ].map((option) => {
            ...
            return (
              ...
              >
                <div className="flex-1">
                  ...
                  <p
                    style={{ fontSize: 12, color: "#64748b", lineHeight: 1.35 }}
                  >
                    {option.description}
                  </p>
                </div>
                <PreferenceSwitch
                  label={option.title}
                  active={active}
                  disabled={!canEditPreferences || savingPreference}
          ...
          {!isLoggedIn && (
            <p className="pt-1" style={{ fontSize: 12, color: "#64748b" }}>
              Entre na sua conta e ative o push para escolher as categorias.
            </p>
          )}
          {isLoggedIn && !loadingPreferences && !canEditPreferences && (
            <p className="pt-1" style={{ fontSize: 12, color: "#64748b" }}>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p
                ...
                style={{ fontSize: 12, color: "#64748b", lineHeight: 1.45 }}
              >
                Usada somente quando você solicita o preenchimento do endereço
        ...
        {(pushBlocked || locationBlocked) && (
          ...
          >
            <div className="flex items-start gap-3">
              ...
              />
              <div>
                <h2 style={{ fontSize: 14, fontWeight: 800, color: "#9a3412" }}>
                ...
                >
                  Abra as configurações do navegador ou do app instalado,
                  procure as permissões deste site e permita
                  {pushBlocked && locationBlocked
